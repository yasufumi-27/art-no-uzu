...
// 同じ画像を使っている作品の組（リンク先だけが違う）。TOP の WORKS / EXHIBITION では show の作品だけを候補にし、hide は出さない。
// /works/ の年別一覧では、それぞれの年にそのまま載せる。
export const SAME_IMAGE_GROUPS = [
  // PSG コラボのサッカーボール：TOP には X（2023_1・PSG Japan の投稿）だけを出す（2026-09-15 指定）。
  // 2022_7 はリンク未登録＝Instagram アカウントへ遷移
  { show: "2023_1", hide: ["2022_7"] },
];
